...
// fp16 V storage (upstream 21a0e85f): V is stored as FP16_RNE(BF16 input).
// gfx906 port: bf16 -> f32 is an exact shift, f32 -> f16 uses the HIP RNE pack.
...
__device__ __forceinline__ std::uint32_t bf16x2_bits_to_f16x2_bits(std::uint32_t bits) {
    return pack_f16x2(__uint_as_float(bits << 16), __uint_as_float(bits & 0xffff0000u));
}
