...
PersistentLayout persistent_layout(const SequencePlanImpl& plan) {
    ...
    if (plan.speculative_backend != SpeculativeBackend::None) {
        ...
        if (plan.speculative_backend == SpeculativeBackend::Mtp &&
            mtp_context_lookup_enabled()) {
            // Same per-device shard as the ordinary records above -- only the column count
            // differs -- so the tp 2 head/channel split has to be applied here too.
...
WorkspacePlan build_workspace_plan(const SequencePlanImpl& plan) {
    ...
    if (plan.features.mtp()) {
        // With the context-lookup path off, every MTP figure below is the pre-port one.
        const std::int32_t lookup_verify =
            mtp_context_lookup_enabled() ? static_cast<std::int32_t>(qwen3_6::kMtpLookupMaximumWidth)
                                         : verify;
        const std::int32_t lookup_drafts =
            mtp_context_lookup_enabled()
                ? static_cast<std::int32_t>(qwen3_6::kMtpLookupMaximumDrafts)
                : drafts;
        ...
        for (std::int32_t batch = 1; batch <= static_cast<std::int32_t>(plan.max_concurrency);
            ...
            tp_mtp_call_roots(alignment, batch * lookup_verify, batch);
            ...
            const std::size_t batch_accept =
                ops::speculative_accept_greedy_drafts_workspace_capacity_bytes(
                    TextConfig::token_domain, lookup_drafts, lookup_drafts, batch, batch);
...
// The 8x graph-allowance scale as an environment knob (NINFER_GFX906_GRAPH_ALLOWANCE_SCALE, 1..64).
// Read once; the default is the value the port tuned at tp=1 (see the allowance comment below).
inline std::size_t gfx906_graph_allowance_scale() {
    static const std::size_t scale = [] {
        const char* text = std::getenv("NINFER_GFX906_GRAPH_ALLOWANCE_SCALE");
        if (text == nullptr || *text == '\0') { return static_cast<std::size_t>(8); }
        char* end = nullptr;
        const long value = std::strtol(text, &end, 10);
        if (end == text || *end != '\0' || value < 1 || value > 64) {
            throw std::invalid_argument(
                "NINFER_GFX906_GRAPH_ALLOWANCE_SCALE must be an integer in [1,64]");
        }
...
// registered native capacity in this build). The single caller passes it straight into the
// sequence plan, so the ceiling is resolved exactly once per Engine construction.
std::uint32_t validate_target_options(DeviceContext& device, const EngineOptions& options) {
    // gfx906 port: --rope native only (YaRN dropped), so the ceiling max_context is admitted
    // against is the variant's registered native capacity.
    const std::uint32_t effective_max_context = kNativeMaxContext;
    if (options.max_context == 0 || options.max_context > effective_max_context) {
...
std::unique_ptr<SequencePlanImpl> build_sequence_candidate(const SequencePlanningInputs& inputs,
    ...
    if (impl->use_cuda_graph) {
        ...
        // NINFER_GFX906_GRAPH_ALLOWANCE_SCALE overrides the 8x default (1..64), for the tp2
        // bring-up: the tp2 cross-device graph holds both devices' nodes (~3x the tp1 count).
        const std::size_t kGraphAllowanceScale = gfx906_graph_allowance_scale();
#else
        ...
        if (impl->speculative_backend == SpeculativeBackend::None) {
            ...
            impl->graph_allowance_bytes = checked_mul(kGraphAllowanceScale * per_batch, impl->max_concurrency,
                                                      "ordinary exact-b graph allowance");
        } else if (impl->speculative_backend == SpeculativeBackend::Mtp) {
            // Two MTP graph families are captured when the context-lookup path is on -- the
            // ordinary five-draft one and the fifteen-draft one -- so both the profile domain and
            // the budget widen. Off, this is the pre-port allowance exactly.
            const bool lookup = mtp_context_lookup_enabled();
            const std::uint32_t allowance_drafts =
                lookup ? qwen3_6::kMtpLookupMaximumDrafts : impl->draft_window;
            const auto profiles = mtp_graph_profiles(impl->capacity, allowance_drafts);
            const std::size_t per_batch_allowance = graph_topology_allowance(
                profiles,
                [&](GraphExecutionProfile profile) {
                    const std::uint64_t final_visible = std::min<std::uint64_t>(
                        impl->capacity,
                        static_cast<std::uint64_t>(profile.max) + 2ULL * allowance_drafts);
                    return kGraphAllowanceScale * (final_visible <= 4096 ? 12ULL : 82ULL) * kMiB;
                },
                "MTP graph allowance");
            impl->graph_allowance_bytes =
                checked_mul((lookup ? 2ULL : 1ULL) * per_batch_allowance, impl->max_concurrency,
