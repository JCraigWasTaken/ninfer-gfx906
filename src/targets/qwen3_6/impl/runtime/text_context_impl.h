...
void TextContext::mtp_prefill_chunk(const Tensor& ids, const Tensor& hidden,
    ...
    if (final_chunk) {
        ...
        ops::rope(last_rope_position, kCfg.rotary_dim, kCfg.rope_theta, qn,
                  s);
...
void TextContext::gdn_mix(const GdnLayerW& w, Tensor& x, int gidx, Phase ph) {
    ...
    if (ph == Phase::Verify) {
        ...
        ops::causal_conv1d_silu_split(qkv, *w.conv1d, conv_state, conv_state, qc, kc, vc, s);
    }
...
void TextContext::attn_mix_tp2(const FullLayerW& w0, const FullLayerW& w1, std::array<Tensor, 2>& x,
    ...
    for_each_rank(execution, [&](int rank) {
        ...
        ops::rope(rope_for_op, kCfg.rotary_dim, kCfg.rope_theta, qn[r], kn[r], s);
...
void TextContext::gdn_mix_tp2(const GdnLayerW& w0, const GdnLayerW& w1, std::array<Tensor, 2>& x,
    ...
    if (ph == Phase::Verify) {
        ...
        for (std::size_t r = 0; r < 2; ++r) {
            // gfx906 port: gdn_prefill_conv yields one buffer; the tp2 path keeps the donor's
            // projected -> convolved pair as two per-rank allocations.
            qkv[r]   = workspace_recipe::gdn_prefill_conv<TextConfig>(*ws[r], T, kTensorParallelWidth);
            qkv_c[r] = workspace_recipe::gdn_prefill_conv<TextConfig>(*ws[r], T, kTensorParallelWidth);
        }
...
void TextContext::mtp_prefill_chunk_tp2(const Tensor& ids, const std::array<Tensor, 2>& hidden,
...

    {
        ...
        for_each_rank(execution, [&](int rank) {
            ...
            ops::rope(rope_positions[r], kCfg.rotary_dim, kCfg.rope_theta, kn, s);
    ...
    for_each_rank(execution, [&](int rank) {
        ...
        ops::rope(last_rope, kCfg.rotary_dim, kCfg.rope_theta, qn, s);
        qwen3_6::PagedKVCacheView pages = rank == 0 ? mtp_kv_ : tp_->mtp_kv;
